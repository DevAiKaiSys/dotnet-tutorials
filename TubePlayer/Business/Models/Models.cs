...
public partial record YoutubeVideo(ChannelData Channel, YoutubeVideoDetailsData Details)
{
    ...
    public string FormattedSubscriberCount =>
        $"{FormatLongNumber(SubscriberCount)} subscriber{(SubscriberCount > 1 ? "s" : string.Empty)}";
    ...
    private TimeSpan PublishedAtAgo => (Details.Snippet?.PublishedAt).GetValueOrDefault(DateTime.Now)
        .Subtract(DateTime.Now);

    private long SubscriberCount =>
        long.TryParse(Channel.Statistics?.SubscriberCount, out var result) ? result : default;
    ...
    {
        var minutesElapsed = Math.Abs(elapsedTime.TotalMinutes);
...
{
    public static YoutubeVideoSet CreateEmpty()
    {
        return new YoutubeVideoSet(ImmutableList.Create<YoutubeVideo>(), string.Empty);
    }
}
