...
public record Calculator
{
    ...
    {
        Calculator calculator = RestartOrClear(key);
        return key switch
        {
    ...
    private static Calculator PlusMinusKey(Calculator calculator)
    {
        if (calculator.HasNumber)
        {
            calculator = calculator with { Number = calculator.Number?.StartsWith("-") == true ? calculator.Number?[1..] : "-" + calculator.Number };
        }
        return calculator;
    }
